...
```{r}
min(resumen$num_ratings)
max(resumen$num_ratings)
```
...
```{r}
mean_pelis <- entrena %>% group_by(movieId) %>%
    summarise(mean_peli = mean(rating), num_calif_peli = n())
mean_pelis <- left_join(mean_pelis, movies)
```
```{r}
max(mean_pelis$num_calif_peli)
min(mean_pelis$num_calif_peli)

quantile(mean_pelis$num_calif_peli)
hist(mean_pelis$num_calif_peli)
```
```{r}
str(mean_pelis)
head(mean_pelis)
#mean_pelis <- data_frame(table(mean_pelis))
```


```{r}
arrange(mean_pelis, desc(mean_peli)) %>%
    filter(num_calif_peli >= 40) %>%
    top_n(20, mean_peli) %>%
    mutate(mean_peli = round(mean_peli, 2)) %>%
    DT::datatable()
